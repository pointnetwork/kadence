...
class ChurnFilterPlugin {
  ...
  _wrapAbstractNodeSend() {
    ...
    this.node.send = (method, params, target, handler) => {
      ...
      send(method, params, target, (err, result) => {
        if (err.type === 'TIMEOUT' || err.dispose) {
  ...
  hasBlock(fingerprint) {
    ...
    if (this.blocks.has(fingerprint)) {
  ...
  /**
   * Clears all blocks and cooldown data
   */
  reset() {
    this.cooldown.clear();
    this.blocks.clear();
  ...
  /**
   * Releases blocks to reset cooldown multipliers for fingerprints with
   * cooldowns that are long expired and not blocked
   */
  resetCooldownForStablePeers() {
    const now = Date.now();

    for (let [fingerprint, cooldown] of this.cooldowns) {
      if (this.hasBlock(fingerprint)) {
        continue;
      }

      let { expired, expiration } = this.cooldowns.get(fingerprint);
