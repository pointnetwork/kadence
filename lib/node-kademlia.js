...
class KademliaNode extends AbstractNode {
  ...
  refresh(startIndex = 0, callback = () => null) {
    ...
    // NB: going to check and only continue to refresh if every refresh
    // NB: yields at least one newly discovered contact.
    let results = new Set(), totalUnimprovedLookups = 0;

    function isDiscoveringNewContacts() {
      return totalUnimprovedLookups < constants.MAX_UNIMPROVED_REFRESHES;
    }
