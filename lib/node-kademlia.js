...
class KademliaNode extends AbstractNode {
   ...
   * @param {string|object} peers[1] - Address data for contact
  ...
  iterativeFindValue(key, callback) {
    ...
    const missing = [];
    const messages = this.router.getClosestContactsToKey(key, constants.K)
    ...
    async.eachLimit(messages, constants.ALPHA, (message, next) => {
      ...
      this.send(...message, (err, result) => {
        ...
        if (Array.isArray(result)) {
          result.forEach((contact) => contacts.set(contact.identity, contact));
          missing.push(target);
          return next();
        }
        ...
        if (closestMissingValue) {
          this.send('STORE', [key, result], closestMissingValue);
        }

        callback(null, result);
      });
    }, () => callback(null, getClosestNodesFromSet([...contacts.entries()])));
  }
  /**
