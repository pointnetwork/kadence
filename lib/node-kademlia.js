...
class KademliaNode extends AbstractNode {
  ...
  /**
   * @typedef {function} KademliaNode~beforeReplicateTransformer
   * @param {object} entry
   * @param {string} key - The DHT entry key
   * @param {KademliaNode~entry} value - The DHT entry value
   */
  ...
  iterativeStore(key, value, callback) {
    ...
    async.waterfall([
      ...
      (next) => this.storage.put(key, this._createStorageItem(value),
