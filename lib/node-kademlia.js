...
class KademliaNode extends AbstractNode {
   ...
   * @private
   */
  _iterativeFind(RPC, key, callback) {
    function createRpc(target) {
      return [RPC, [key], target];
    }
    ...
    function iterativeLookup(selection, callback, continueLookup = true) {
      ...
      async.detectLimit(selection, constants.ALPHA, (contact, next) => {
        ...
        // NB: if we have a result, then we found a value to call back with
