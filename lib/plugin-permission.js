...
class PermissionRules {
  ...
  store(request, response, next) {
    ...
    try {
      solution = new PermissionSolution(Buffer.from(item.value[0]));
      value = item.value[1];
...
class PermissionWallet extends EventEmitter {
   ...
   * @param {PermissionSolution} solution
   */
  put(solution) {
...
class PermissionPlugin {
   ...
   * @param {buffer} value
   * @param {PermissionPlugin~iterativeStoreCallback} callback
   */
  ...
  /**
   * @callback PermissionPlugin~iterativeStoreCallback
